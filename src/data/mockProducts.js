...
// Hàm lấy tất cả các style_id từ database
const styles = [
...
async function generateMockProducts(num = 10) {
  ...
  for (let i = 0; i < num; i++) {
    ...
    const randomStyle = chance.pickone(styles); // Random style
    ...
    products.push({
      ...
      price: chance.integer({ min: 10000, max: 20000 }), // Random price
      transmission: randomTransmission.value, // Random transmission type
      status: randomStatus.value, // Random status
      year: randomYear.value, // Convert year to a number
